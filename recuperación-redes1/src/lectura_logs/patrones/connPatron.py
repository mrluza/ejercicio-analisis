...
Patron de "conexion" (capa 2, Ethernet). A diferencia de los patrones del
curso 2025, este NO usa extraer_todo(): filtra a mano una lista concreta de
campos de la capa eth (dict_values). Se deja tal cual como ejemplo del estilo
"antiguo" de patron, en el que cada patron elegia sus propios campos.
'''
...
class connPatron(PatronPadre):
    ...
    '''
    # Lista de campos que nos interesan de la capa Ethernet. Solo se guardaran
    # estos (a diferencia de los patrones nuevos, que guardan la capa entera).
    ...
    def process_log_data(self, data_string):
        # Arrancamos con un diccionario vacio que tiene una clave por cada
        # campo de dict_values.
        ...
        # Recorremos los campos que nos interesan y, si aparecen en la capa
        # eth del paquete, los copiamos (limpiando el prefijo de tshark).
        ...
        # Si no hemos encontrado nada de la capa eth, el paquete no interesa.
        if not has_data:
            resultado = None
        return resultado
